...
import {evaluatePersonalConsequence} from "./value-core.js";
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const [profiles,goals,financeRows,caps,snapRows]=await Promise.all([
   ...
   query("goals","user_id=eq."+uid+"&status=eq.active&select=id,title,domain,current_value,target_value,target_unit,target_date&order=priority.asc,created_at.asc&limit=1",jwt),
  ...
  const [contracts,candidates]=await Promise.all([
   goal?query("goal_contracts","user_id=eq."+uid+"&goal_id=eq."+goal.id+"&select=desired_state,guardrails,success_criteria&order=updated_at.desc&limit=1",jwt):Promise.resolve([]),
  ...
  const value=evaluatePersonalConsequence({profile,goal,contract:contracts?.[0]||null,finance,
  ...
  return J({ok:true,generated_at:new Date().toISOString(),candidate_id:candidate?.id||null,
    goal_id:goal?.id||null,personal_value:value,
